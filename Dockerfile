...
# Install torch FIRST (critical for detectron2)
...
# Install detectron2 (now torch is available)
...
# Install web interface and utilities
RUN pip install --no-cache-dir \
    gradio==4.44.1 \
    omegaconf==2.3.0 \
    pyyaml==6.0.1 \
...
# Install color science and additional utilities
RUN pip install --no-cache-dir \
    colorspacious==1.1.2 \
